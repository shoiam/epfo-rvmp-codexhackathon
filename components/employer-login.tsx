...
export function EmployerLogin() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/employer/auth/login", {
      ...
      body: JSON.stringify({ entityId, passcode }),
    });
  ...
  return (
    <main className="employer-page">
      <section className="employer-login-card">
        <p className="employer-kicker">Internal HR tool</p>
        <h1>Employer portal</h1>
        <p className="employer-muted">
          Review declarations and exit requests for your establishment.
        </p>
        <form className="employer-form" onSubmit={submit}>
          <label htmlFor="entityId">Establishment entity ID</label>
          <input
            id="entityId"
            value={entityId}
            onChange={(event) => setEntityId(event.target.value)}
            placeholder="e.g. MHBAN0045612000"
            required
          />
          ...
          <input
            ...
            required
          />
          <button className="employer-button" disabled={busy} type="submit">
        ...
        <p className="employer-hint">
          Demo passcode: <code>demo1234</code>
        </p>
      </section>
    </main>
  );
