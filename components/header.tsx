    ...
    <header className="flex h-16 items-center justify-between border-b border-slate-200 bg-white px-5 lg:px-8">
      ...
      <details className="relative">
        <summary className="flex cursor-pointer list-none items-center gap-2 rounded-md px-2 py-1.5 text-sm text-slate-700 hover:bg-slate-100">
          ...
          <span className="hidden text-left sm:block"><span className="block text-xs text-slate-500">UAN</span><span className="font-medium">Not signed in</span></span>
          <ChevronDown className="h-4 w-4" />
        </summary>
        <div className="absolute right-0 z-20 mt-2 w-56 rounded-md border border-slate-200 bg-white p-2 shadow-lg">
          <p className="px-3 py-2 text-xs text-slate-500">UAN: Available after sign-in</p>
          ...
          <Link href="/login" className="flex items-center gap-2 rounded px-3 py-2 text-sm text-red-700 hover:bg-red-50"><LogOut className="h-4 w-4" />Logout</Link>
        </div>
      </details>
    </header>
  );
