...
export default async function DashboardServiceHistoryPage() {
  ...
  const data = memberships.map((membership) => {
    ...
    const daysUntilAutoAccept =
      exitRequestedAt && membership.exitConfirmedAt === null
        ? Math.max(
            0,
            Math.ceil((10 * DAY_MS - (now.getTime() - exitRequestedAt.getTime())) / DAY_MS),
          )
        : null;
    const verificationRequestedAt = membership.verificationRequestedAt;
    const verificationPending =
      verificationRequestedAt !== null &&
      membership.verificationConfirmedAt === null &&
      effectiveVerification !== "VERIFIED";
    const verificationDaysRemaining = verificationPending
      ? Math.max(
          0,
          Math.ceil((10 * DAY_MS - (now.getTime() - verificationRequestedAt.getTime())) / DAY_MS),
        )
      : null;
    return {
      id: membership.id,
      establishment: membership.establishment,
      doj: membership.doj.toISOString(),
      doe: membership.doe?.toISOString() ?? null,
      exitReason: membership.exitReason,
      effectiveStatus,
      source: membership.source,
      effectiveVerification,
      concurrentAcknowledgedAt: membership.concurrentAcknowledgedAt?.toISOString() ?? null,
      exitRequestedAt: membership.exitRequestedAt?.toISOString() ?? null,
      exitConfirmedAt: membership.exitConfirmedAt?.toISOString() ?? null,
      autoAccepted: pendingExit && effectiveStatus === "ENDOFSERVICE",
      daysUntilAutoAccept,
      verificationPending,
  ...
  for (let left = 0; left < activeMemberships.length && !concurrentWarning; left += 1) {
    for (let right = left + 1; right < activeMemberships.length; right += 1) {
      ...
      const overlapEnd = Math.min(
        a.doe ? new Date(a.doe).getTime() : now.getTime(),
        b.doe ? new Date(b.doe).getTime() : now.getTime(),
      );
      if (
        overlapEnd - overlapStart > 60 * DAY_MS &&
        !a.concurrentAcknowledgedAt &&
        !b.concurrentAcknowledgedAt
      )
        concurrentWarning = `${a.establishment.name} and ${b.establishment.name}`;
    }
  }
