...
export async function POST(req: Request, { params }: { params: Promise<{ claimId: string }> }) {
  ...
  const { claimId } = await params; const claim = await prisma.claim.findFirst({ where: { id: claimId, userId }, include: { stages: true } }); if (!claim) return NextResponse.json({ error: "Claim not found." }, { status: 404 });
