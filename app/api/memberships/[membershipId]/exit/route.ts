...
const exitReasons = new Set(["Resignation", "Retirement", "Termination", "End of contract", "Death", "Other"]);
...
export async function POST(request: Request, { params }: { params: Promise<{ membershipId: string }> }) {
  ...
  if (!exitDate || Number.isNaN(exitDate.getTime()) || !exitReason || !exitReasons.has(exitReason)) {
  ...
  if (getEffectiveStatus(membership) !== "ACTIVE") {
    return NextResponse.json({ message: "An exit can only be logged for an active membership." }, { status: 409 });
  }
  await prisma.membership.update({ where: { id: membership.id }, data: { doe: exitDate, exitReason, exitRequestedAt: new Date(), exitConfirmedAt: null } });
